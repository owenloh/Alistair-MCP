...
small agent on {user}'s laptop (online-only); the draft tool returns a wa.me link and
NEVER sends.
"""

CHATS = (
    "List {user}'s recent WhatsApp chats (chat id/jid, name, last-message time, unread). "
...
READ = (
    ...
    "Read-only, online-only (via the laptop agent). Summarise for {user}; it is their private "
    "messaging — never repeat secrets, OTP/2FA codes, or passwords you see."
)

SEARCH = (
    "Search {user}'s WhatsApp messages by text. Read-only, online-only (via the laptop agent). "
...
DRAFT = (
    "Draft a WhatsApp message — returns a wa.me link that opens {user}'s NORMAL WhatsApp with "
    "the text PRE-FILLED in the compose box for them to review and SEND THEMSELVES. It NEVER "
    ...
    "agent if it's online), and 'body' as the message. 1:1 chats only. Write in {user}'s voice, "
    "keep it tight, then hand them the link."
)
