...
| **MCP wrap** — Streamable-HTTP + OAuth, everything-as-tools | ✅ **built, tested & live-verified** (74 checks) | `app/mcp_server.py` + `app/mcp_oauth.py`; `alistair_assistant` at `/mcp`, 22 persona-described tools, auto-approve OAuth 2.1 (DCR + PKCE) + SERVICE_API_KEY bearer. **Full DCR→authorize→token→authenticated-/mcp flow verified live on Railway**; only the claude.ai connect UI remains (your action) |
