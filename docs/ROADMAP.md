...
- **WhatsApp connector** ✅ live on `main` — `/api/whatsapp/*` (6) + 6 MCP tools (`whatsapp_chats`, `whatsapp_read`, `whatsapp_search`, **`whatsapp_recent`** (inbox), **`whatsapp_find`** (resolve a name/number → read the chat in one hop), `whatsapp_draft`). Mirrors the Gmail read+draft, never-sends model. Totals now **60 MCP tools / 82 REST endpoints / 9 served skills**; `tests/test_whatsapp.py` (45 checks), full suite green. **Live-verified on Railway:** `whatsapp_recent` returns an inbox with last-message previews; `whatsapp_find('chloe')` resolved to the right contact + UK number and read the chat in one hop (the exact failure that previously needed the user to type the number); draft returns a `wa.me` link.
- **Architecture (privacy-first):** the WhatsApp session is NOT in the MCP. A separate **laptop read-agent** (**Go + whatsmeow + SQLite**, its own linked device; repo `owenloh/whatsapp-agent`) holds the session, **resolves identity** (canonical phone-number JIDs — no raw `@lid` — + saved contact names), and keeps **persistent local history** (SQLite, ~180d / 5k per chat). It exposes a Bearer-authed HTTP API (`/health, /qr, /chats, /messages, /search, /contacts, /recent, /resolve`); the **stateless** MCP proxies reads over **Tailscale** (`WHATSAPP_AGENT_URL` + `WHATSAPP_AGENT_SECRET`). Online-only: laptop off → clean "offline". Drafting needs no agent — a `wa.me/<number>?text=…` link the user taps to send themselves (1:1 only). Nothing is stored in the cloud. *(Initial build was Node + Baileys; rebuilt to whatsmeow+SQLite to fix LID/identity resolution — HTTP contract unchanged, so the MCP tools were untouched.)*
...
- **Cloud history mirror (offline reads)** 📋 (lower priority since the rebuild) — the laptop's local SQLite now gives persistent, searchable history while it's **on**, so a cloud mirror is only needed for the **laptop-off** case. Store a bounded WhatsApp history on the MCP side for that. **Tradeoff:** message content then rests in the cloud (your own Railway, but still) — it reverses the privacy reason we self-hosted, so keep it tight. Design we landed on:
...
- **Agent cold-start chats** ✅ **resolved by the whatsmeow+SQLite rebuild** — persistent local store + identity resolution means `whatsapp_chats` / `whatsapp_recent` return real, named chats immediately (no longer empty until a new message arrives).
- **Identity resolution (LID → number/name)** ✅ **resolved by the rebuild** — chats now surface canonical phone numbers (no raw `@lid`) and saved contact names; `whatsapp_find` / `/resolve` turn "read my chat with Chloe" into one hop. (Was the root cause of the "Chloe shows September / can't find her UK number" failure.)
- **Local history store in the agent** ✅ **done** — the agent persists to SQLite (~180d / 5k per chat), so reads survive restarts and search spans history.
...
- **Inbox/resolve verbs** ✅ **done** — `whatsapp_recent` (inbox + previews) and `whatsapp_find` (name/number → chat) shipped and live-verified; name-aware draft via `/resolve`.

---

## Latest (2026-06-24) — Gmail, self-contained MCP, safe-write fix

- **Gmail (read + draft, never sends)** ✅ live — `/api/gmail/*` (6) + 6 MCP tools (`gmail_search`, `gmail_read_thread`, `gmail_list_drafts`, `gmail_create_draft`, `gmail_update_draft`, `gmail_delete_draft`). Rides the same Google token (scopes `gmail.readonly` + `gmail.compose`). Live-verified.
...
confirm against the real workspace (held for the user's ok — the dry_run already proved the anchor);
...
| Tools wired (domain + persona + memory) with persona descriptions | ✅ **built, tested & live** — **28 tools** (load_context, get/save_memory, get_skill, daily_brief, project_context, save_reference, add_action, notion_*, calendar_*, intray, github_*, **gmail_***), each Alistair-voiced; safety hooks duplicated into descriptions; all in-process over the existing services. `notion_update_page` exposes `content_updates` so the safe targeted edit works over MCP. |
